...
#        USAGE: ./day16.pl  
#
#  DESCRIPTION: Day 16 - An Aunt named Sue
#
#      OPTIONS: ---
# REQUIREMENTS: ---
#         BUGS: ---
...
while ( <DATA> ) {
    ...
    my @info = split /[:,]?\s+/;
    my $number = $info[1];
    my %attributes = @info[ 2 .. $#info ];
    my $success = 1;

    $success &&= $attributes{$_} == $detected{$_} for keys %attributes;
    $part1 = $number if $success;
    $success = 1;

    for my $attribute ( keys %attributes ) {
        given ( $attribute ) {
            $success &&= $attributes{$attribute} > $detected{$attribute} when 'cats';
            $success &&= $attributes{$attribute} > $detected{$attribute} when 'trees';
            $success &&= $attributes{$attribute} < $detected{$attribute} when 'pomeranians';
            $success &&= $attributes{$attribute} < $detected{$attribute} when 'goldfish';

            default {
                $success &&= $attributes{$attribute} == $detected{$attribute};
            }
        }
    }
    $part2 = $number if $success;
}
