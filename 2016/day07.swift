...
class IPv7 {
    ...
    init?(line: Substring) {
        ...
        var regions = line.split(separator: "[", omittingEmptySubsequences: false)
        let first = regions.removeFirst()
        
        if first != "" {
            if first.contains("]") { return nil }
            superList.append(first)
        }
        
        for next in regions {
            if !next.contains("]") { return nil }
            
            let subregions = next.split(separator: "]", omittingEmptySubsequences: false)
            
            if subregions.count != 2 { return nil }
            
            hyperList.append(subregions[0])
            superList.append(subregions[1])
        }
...
for line in lines {
    if let ip = IPv7( line: line ) {
        if ip.supportsTLS() { countTLS += 1 }
        if ip.supportsSSL() { countSSL += 1 }
    }
}

print( "Part1:", countTLS )
print( "Part2:", countSSL )
