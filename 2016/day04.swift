...
func isValid(line: [Substring] ) -> Int {
    let chunks = line.prefix(line.count - 2)
    let checksum = line.last!
    let sectorID = Int( line[ line.count - 2 ] )!
    ...
    let sorted = histo.sorted { $0.1 > $1.1 ? true : ( $0.1 < $1.1 ? false : ( $0.0 < $1.0 ) ) }
    let check = String( sorted.prefix(5).map( { $0.key } ) )

    if check == checksum {
        let words = chunks.map { String( $0.map { decrypt( letter: $0, key: sectorID ) } ) }
        let room = words.joined(separator: " ")
        
        if room.contains("north") {
            print( "Part2: \(sectorID) - \(room)" )
        }
        return  sectorID
    }
...
print( "Part1:", lines.reduce( 0, { $0 + isValid(line: $1) } ) )
