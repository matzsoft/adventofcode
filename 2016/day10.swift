...
class Rule {
    ...
    let highID: Substring
    
    init( lowType: DestinationType, lowID: Substring, highType: DestinationType, highID: Substring ) {
...
class Bot {
    ...
    func take(chip: Int, queue: inout [Bot]) -> Void {
        chips.append(chip)
        if chips.count == 2 {
            queue.append(self)
        }
...
var bots: [Substring:Bot] = [:]
var bins: [Substring:[Int]] = [:]
...
for line in lines {
    ...
    case "bot":
        let bot = words[1]
        let lowID = words[6]
        let lowType = words[5] == "bot" ? DestinationType.bot : DestinationType.bin
        let highID = words[11]
        let highType = words[10] == "bot" ? DestinationType.bot : DestinationType.bin
...
while queue.count > 0 {
    ...
    switch rule.lowType {
    case .bot:
        bots[rule.lowID]?.take(chip: low, queue: &queue)
    case .bin:
        let bin = rule.lowID
    ...
    case .bot:
        bots[rule.highID]?.take(chip: high, queue: &queue)
    case .bin:
        let bin = rule.highID
...
let ID0 = Substring( String(0) )
let ID1 = Substring( String(1) )
let ID2 = Substring( String(2) )

print( "Part2:", bins[ID0]![0] * bins[ID1]![0] * bins[ID2]![0] )
