...
//  day22
//
//  Created by Mark Johnson on 1/6/19.
...
class Disk {
    ...
    init( position: Point, size: Int, used: Int ) {
...
class Grid {
    ...
    init( input: String ) {
        ...
        for line in lines {
            ...
            case "/dev/grid/node":
                ...
                layout[y].append( Disk( position: Point( x: x, y: y ), size: size, used: used ) )
            default:
                break
            }
        }
    ...
    func canMove( to: Disk, direction: Direction ) -> Disk? {
        guard let from = self[ to.position.move(direction: direction) ] else { return nil }
        
        return from.canMove( to: to ) ? from : nil
    }
    
    func keepMoving( empty: inout Disk, direction: Direction, moves: inout Int ) -> Void {
        while let full = canMove( to: empty, direction: direction ) {
            full.move( to: empty )
            empty = full
            moves += 1
        }
    ...
    func keepMoving( blocked: Direction, empty: inout Disk, direction: Direction, moves: inout Int ) -> Void {
        while canMove( to: empty, direction: blocked ) == nil {
            if let full = canMove( to: empty, direction: direction ) {
                full.move( to: empty )
                empty = full
                moves += 1
            }
    ...
    func moveSequence( empty: inout Disk, sequence: [Direction], moves: inout Int ) -> Void {
    ...
    func summary() -> Void {
        let empties = layout.flatMap {$0}.filter { $0.used == 0 }
        ...
        for y in 0 ..< layout.count {
            ...
            for x in 0 ..< layout[y].count {
                if let disk = self[ Point(x: x, y: y) ] {
                    if disk.used == 0 {
                        line += "_"
    ...
    func shuffle() -> Int {
        ...
        keepMoving( empty: &empty, direction: .up, moves: &moves )
        keepMoving( blocked: .up, empty: &empty, direction: .left, moves: &moves )
        keepMoving( empty: &empty, direction: .up, moves: &moves )
        keepMoving( empty: &empty, direction: .right, moves: &moves )

        while empty.position.x > 1 {
            moveSequence(empty: &empty, sequence: [ .down, .left, .left, .up, .right ], moves: &moves)
        }
...
//grid.summary()
print( "Part1:", grid.countViable() )
print( "Part2:", grid.shuffle() )
