...
#        USAGE: ./day06.pl  
#
#  DESCRIPTION: Day 6 - "Megapixel" your house.
#
#      OPTIONS: ---
# REQUIREMENTS: ---
#         BUGS: ---
...
my @old_lights = map { [ map { 0 } 0 .. 999 ] } 0 .. 999;
my @new_lights = map { [ map { 0 } 0 .. 999 ] } 0 .. 999;
...
while ( <DATA> ) {
    ...
    if ( @words > 4 ) {
        splice @words, 0, 2, "$words[0] $words[1]";
    }

    given ( $words[0] ) {
        affect( $words[1], $words[3], \&old_turn_off, \&new_turn_off ) when 'turn off';
        affect( $words[1], $words[3], \&old_turn_on, \&new_turn_on ) when 'turn on';
        affect( $words[1], $words[3], \&old_toggle, \&new_toggle ) when 'toggle';

        default {
            die "Invalid command $words[0] on line $.\n";
        }
    }
}

say sum( map { sum @$_ } @old_lights );
say sum( map { sum @$_ } @new_lights );


sub affect {
    my ( $first, $last, $old_function, $new_function ) = @_;
    my ( $fx, $fy ) = split /,/, $first;
    my ( $lx, $ly ) = split /,/, $last;

    for my $x ( $fx .. $lx ) {
        for my $y ( $fy .. $ly ) {
            $old_function->( $old_lights[$x][$y] );
            $new_function->( $new_lights[$x][$y] );
        }
    }
}
