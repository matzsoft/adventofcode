...
func parse( input: String ) -> [Int] {
    ...
    var seen: [Position:Int] = [current:0]
    var stack: [Position] = []
    var directions: [Character:Position] = [
        "N" : Position(x: 0, y: -1),
        "E" : Position(x: 1, y: 0),
        "S" : Position(x: 0, y: 1),
        "W" : Position(x: -1, y: 0)
    ...
    for char in input[ input.index(after: input.startIndex) ... input.index(input.endIndex, offsetBy: -2)] {
        switch char {
        case "N", "E", "S", "W":
            let last = current
            
            current = current + directions[char]!
            if let old = seen[current] {
                seen[current] = min( old, seen[last]!+1 )
            } else {
                seen[current] = seen[last]!+1
            }
        case "(":
...
print( "Test1:", parse(input: test1).max()! )
print( "Test2:", parse(input: test2).max()! )
print( "Test3:", parse(input: test3).max()! )
print( "Test4:", parse(input: test4).max()! )
print( "Test5:", parse(input: test5).max()! )

let counts = parse(input: input)
print( "Part1:", counts.max()! )
print( "Part2:", counts.filter { $0 >= 1000 }.count )
