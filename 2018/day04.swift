...
//  day04
//
//  Created by Mark Johnson on 12/3/18.
...
for line in lines {
    ...
    let minute = Int( words[1].split( whereSeparator: { ":]".contains($0) } )[1] )!
    let observation = words[2]
    ...
    case "Guard":
        ...
        let index = words[3].index(words[3].startIndex, offsetBy: 1)
        
        watchman = Int( words[3][index...] )!
        status = .awake
        
        if guardsInfo[watchman] == nil {
            guardsInfo[watchman] = GuardInfo()
        }
    case "falls":
        switch status {
        case .asleep:
...
let biggest = guardsInfo[culprit]!.histo.max()
let target = guardsInfo[culprit]!.histo.enumerated().first(where: { $0.element == biggest } )?.offset

print( "Part1:", culprit * target! )

func findBest( key: Int, info: GuardInfo ) -> ( key: Int, minute: Int, count: Int ) {
    let ( minute, count ) = info.histo.enumerated().max(by: { $0.element < $1.element } )!
    
    return ( key: key, minute: minute, count: count )
}

let pbests = guardsInfo.map( { findBest(key: $0.key, info: $0.value) } )
let ( key, minute, count ) = pbests.max(by: { $0.count < $1.count } )!

print( "Part2", key * minute  )
