...
func parse( input: String ) -> [Light] {
    ...
    for line in lines {
        ...
        let x = Int( words[1] )!
        let y = Int( words[2] )!
        let light = Light( x: x, y: y, dx: Int( words[4] )!, dy: Int( words[5] )! )
...
func stepLights( lights: [Light] ) -> Void {
    for light in lights {
        light.forward()
    }
}

func backLights( lights: [Light] ) -> Void {
    for light in lights {
        light.backward()
    }
}
...
func yRange( lights: [Light] ) -> Int {
    let ymin = lights.min(by: { $0.y < $1.y } )!.y
    let ymax = lights.max(by: { $0.y < $1.y } )!.y

    return abs( ymax - ymin )
}

func printLights( step: Int, lights: [Light] ) -> Void {
    let xmin = lights.min(by: { $0.x < $1.x } )!.x
    let xmax = lights.max(by: { $0.x < $1.x } )!.x
    let ymin = lights.min(by: { $0.y < $1.y } )!.y
    let ymax = lights.max(by: { $0.y < $1.y } )!.y
    ...
    for y in ymin ... ymax {
        ...
        for x in xmin ... xmax {
            line.append( lights.contains(where: { $0.x == x && $0.y == y } ) ? "#" : "." )
        }
