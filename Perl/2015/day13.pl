...
#        USAGE: ./day13.pl  
#
#  DESCRIPTION: Day 13 - The Round Table of Bob & Carol & David (WTF?) & Alice
#
#      OPTIONS: ---
# REQUIREMENTS: ---
#         BUGS: ---
...
while ( <DATA> ) {
    ...
    my ( $receiver, undef, $direction, $count, undef, undef, undef, undef, undef, undef, $giver ) = split;
    
    substr $giver, -1, 1, q{};
    $count = -$count if $direction eq 'lose';
    $network{$receiver}{$giver} = $count;
}

my @names = sort keys %network;

say max get_arrangements( [ $names[0] ], [ @names[ 1 .. $#names ] ] );

$network{me} = { map { $_ => 0 } @names };
$network{$_}{me} = 0 for @names;
say max get_arrangements( [ $names[0] ], [ @names[ 1 .. $#names ], 'me' ] );
...
sub get_arrangements {
    ...
    if ( @remaining == 1 ) {
        ...
        push @table, @remaining;

        for my $index ( 0 .. $#table - 1 ) {
            $count += $network{ $table[$index] }{ $table[ $index - 1 ] };
            $count += $network{ $table[$index] }{ $table[ $index + 1 ] };
        }

        $count += $network{ $table[-1] }{ $table[-2] };
        $count += $network{ $table[-1] }{ $table[0] };

        return ( $count );
    }

    for my $next ( @remaining ) {
        push @arrangements, get_arrangements( [ @table, $next ], [ grep { $_ ne $next } @remaining ] );
    }
