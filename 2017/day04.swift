...
//  day04
//
//  Created by Mark Johnson on 1/10/19.
...
for line in lines {
    ...
    let normalized = words.map { String( $0.sorted() ) }
    let wordSet = Set( words )
    let anagramSet = Set( normalized )
    
    if words.count == wordSet.count { part1 += 1 }
    if words.count == anagramSet.count { part2 += 1 }
}
