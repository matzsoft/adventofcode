...
func parse( input: String ) -> [String:Program] {
...
func treeDump( tower: [ String: Program ], root: Program, prefix: String = "", width: Int = 0 ) -> Void {
    let weightString = String(format: "%*d", width, root.weight )
    
    print( "\(prefix)\(weightString) \(root.name)" )
    if root.children.count > 0 {
        let prefix = prefix + String( repeating: " ", count: weightString.count - 1 ) + "| "
        let width = String( root.children.map { tower[$0]!.weight }.max()! ).count
        
        root.children.forEach { treeDump( tower: tower, root: tower[$0]!, prefix: prefix, width: width ) }
    }
}

func balance( tower: [ String: Program ], disc: Program ) -> ( name: String, totalWeight: Int ) {
    let weights = disc.children.map { balance( tower: tower, disc: tower[$0]! ) }.sorted {
        $0.totalWeight < $1.totalWeight
    }
    
    if weights.count > 1 {
        if weights.first!.totalWeight != weights.last!.totalWeight {
            let small = weights.first!
            let big = weights.last!
            
            if small.totalWeight == weights[1].totalWeight {
                print( "Part2:", tower[big.name]!.weight - big.totalWeight + small.totalWeight )
            } else {
                print( "Part2:", tower[small.name]!.weight + big.totalWeight - small.totalWeight )
            }
            exit(1)
        }
    }
...
if list.count == 0 || list.count > 1 { print( "Big Trouble in Little China" ); exit(1) }

print( "Part1:", list[0].name )
//treeDump( tower: tower, root: tower["yruivis"]! )
_ = balance( tower: tower, disc: list[0] )
